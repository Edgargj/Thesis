...
Estos siete pasos se utilizan para ensamblar una energía molecular como la suma de varias diferencias de energía y un incremento de energía empírico final (la ``corrección de nivel superior’’) en función del número de electrones apareados y no apareados. Debido a las correcciones de energía empírica en los métodos  gaussianos, no son completamente ab initio, sino algo semiempíricos, excepto cuando estas correcciones se anulan. Esta cancelación ocurre, por ejemplo, al calcular las afinidades de los protones como la diferencia de energía de las especies protonadas y no protonadas. Los métodos gaussianos, y otros no mencionados aquí, así como los métodos CBS son revisados  por Peterson, Feller y Dixon (2012), con énfasis en termoquímica; estos autores reconocen una precisión  química de 4 kJ mol$^{-1}$ (1 kJ mol$^{-1}$) como aceptable \cite{Peterson2012}. A continuación se muestra una comparación de los métodos gaussianos y un método CBS, usando 1,4-benzoquinona (p-benzoquinona, $O=C_{6}H_{4}=O$) al calcular la entalpía de formación por el método de atomización, se debe tener en cuenta que el cálculo G4 tomó nueve veces más tiempo (160/18) que el G4 (MP2) y ambos dieron casi la misma entalpía de formación (veáse la tabla \ref{Fattahi-table}). 
...
\end{tabular} 
\end{center}
\caption{La entalpía de formación aceptada en la literatura es de -122. $\pm$ 3.8 kJ mol$^{1}$ \cite{Fattahi2005}. Los primeros cuatro valores calculados experimentalmente están dentro de 1 kJ mol$^{-1}$, sin embargo, el valor CBS-QB3 es 3 kJ mol$^{-1}$ por encima del mayor error estimado. No obstante, no se debe generalizar a partir de una muestra de un solo compuesto.}
\label{Fattahi-table}
\end{table}

\section{Métodos CBS}
La clave de estos métodos es la extrapolación del conjunto de bases a un límite infinito (hasta el final). Hay tres métodos CBS básicos: CBS-4 (para extrapolación de cuarto orden), CBS-Q (para CI cuadrático) y CBS-APNO (para orbitales naturales de pares asintóticos, referidos a la extrapolación al límite establecido de base). Estos métodos están disponibles en los programas Gaussian 94 y posteriores. CBS-4M puede manejar
...
\item Determinación de punto único de HF con un conjunto de bases muy grande (6–311 + G(3d2f,2df,p) o 6–311 + G(3d2f,2df,2p), dependiendo del método CBS en particular).

\item Cálculo de punto único MP2 (base que depende del método CBS particular).
...
\item Una o más correcciones empíricas. Se debe tener en cuenta el aspecto semiempírico de los métodos CBS. 

\end{enumerate}

\begin{comment}
\section{Comparacion entre métodos} 
Nos concentraremos en los métodos de tipo gaussiano y CBS, porque estos han sido los más utilizados y por lo tanto han acumulado un archivo de resultados, son los más accesibles y porque hay varias versiones disponibles. Sin embargo, existen otros métodos multipaso de alta precisión, como los procedimientos de Weizmann de Martin y de Oliveira, W1 y W2 [189], y de Boese et al., W3 y W4 [190a] y las modificaciones más rápidas de estos como W2X y W3X-L de Chan y Radom [190b], que al igual que los métodos CBS se basan en la extrapolación de conjuntos de bases. W1 y W2 tienen una desviación absoluta media para energías relativas de aproximadamente 1 kJ mol "1 (no 1 kcal mol "1), e incorporan efectos relativistas, y W2 no tiene parámetros empíricos, a diferencia de los métodos gaussiano y CBS. Los métodos W3 y W4 tienen errores similares a los de W1 y W2, y los autores especulan sobre las razones de la obstinada “barrera de 0,1 kcal/mol”. Algunos de estos métodos de Weizmann (W1, W2X, W1X-1, W1X-2), se pueden utilizar para moléculas de hasta 10 o 12 átomos pesados. Consulte la revisión de métodos de varios pasos de alta precisión de Peterson et al. [185]. Nota:
el autor principal de esta revisión es el profesor K. A. Peterson de la Universidad Estatal de Washington; los métodos CBS provienen principalmente del grupo de investigación del profesor G. A. Petersson (ortografía diferente) de la Universidad Wesleyana; ambos investigadores están activos en termoquímica cuántica computacional.\\

De los métodos de tipo gaussiano y CBS, para una alta precisión en moléculas muy pequeñas, CBS-APNO es la elección adecuada, y para moléculas "grandes", la elección recae en CBS-4M con la aceptación de la posibilidad de errores moderadamente grandes. Para moléculas de tamaño intermedio, la mejor elección es probablemente entre G4(MP2) y CBS-QB3. Recuerde que G4(MP2) es mucho más rápido que G4, con poca pérdida de precisión en la mayoría de los casos. Dentro de estos límites, la pregunta será si usar G4 (MP2) o CBS-QB3. Cuál, si alguno, tiene la ventaja solo se puede encontrar comparando cálculos con experimentos para las propiedades y los tipos de moléculas de interés. Aquí hay algunos ejemplos de estudios que utilizan estos métodos. Pokón et al. comparó CBS-QB3, CBS-APNO y G3 (este último presumiblemente similar a G3 (MP2) y un poco menos preciso que G4 (MP2) para tales cálculos) para las entalpías y energías libres de reacción de desprotonación en fase gaseosa ciones y descubrió que "La combinación de alta precisión y costo computacional relativamente bajo hace que el método CBS-QB3 sea la mejor opción de los tres" (los tres dieron una desviación absoluta media del experimento de aproximadamente 1 kcal mol "1, es decir, aproximadamente 4 kJ mol "1 ) [191]. Bond comparó G2, G2(MP2), G3, G3(MP2), G3(B3), G3(MP2B3), CBS-QB3 y DFT para el cálculo de entalpías y energías libres de formación de casi 300 compuestos orgánicos y encontró que G3 era mejor con G3 (MP2) un poco peor; CBS-QB3 también fue preciso pero más limitado en el tamaño de moléculas que podría manejar [192]. Las desviaciones absolutas medias para esos tres métodos que utilizan una reacción isodésmica (consulte la Sección 5.5.2.3.3) fueron (kJ mol "1):\\
