...
La clave de estos métodos es el uso de altos niveles de correlación y grandes conjuntos de bases. Esta serie comenzó en 1989 con G1 [177], continuó con G2 (1991) [179] y G3 (1998) [180], y por último, se publicó G4 (2007) [181]. Respecto a G1 y G2, están obsoletos. Los métodos gaussianos de alta precisión más populares en la actualidad son probablemente G4, G3 y sus variantes más rápidas pero casi tan precisas, G4(MP2) [182] y G3(MP2) [183].\\ 
...
\begin{comment}
Para G3, la desviación absoluta promedio del experimento es $1.3 kcal\cdot mol^{-1}$ y para G3(MP2) $1.2-1.3 kcal\cdot mol^{-1}$ y G3 (MP2) parece ser 7-8 veces más rápido que G3 [183]. Curtis y otros autores proporcionaron los detalles del método G4 [181] y lo compararon con G3, en cierta medida, con G1 y G2. Ellos informaron que ``[...]la desviación absoluta promedio del experimento muestra una mejora significativa de $1.3 kcal\cdot mol^{-1}$ (teoría G3) a $0.3 kcal\cdot mol^{-1}$ (teoría G4)''. G4 es de 2 a 3 veces más lento que G3, para acelerar el método G4, sus pasos MP4 fueron reemplazados con MP2 y MP3 dando G4 (MP2) y G4 (MP3) [182]. Estos tienen desviaciones  absolutas del experimento de $1.4 kcal\cdot mol^{-1}$ y $1.3 kcal\cdot mol^{-1}$. El método G4(MP2) parece en general ser el mejor de los dos; es de 2 a 3 veces más rápido que G3, aunque aproximadamente es el doble de lento que G3 (MP2), Curtis y otros autores [182] mencionan que ``En general, el método G4 (MP2) proporciona un método preciso y económico para las predicciones termodinámicas''. Tiene una precisión general para el conjunto de pruebas de moléculas G3/05 que es significativamente mejor que la teoría G3(MP2) ($1.04$ vs $1.39 kcal\cdot mol^{-1}$ e incluso mejor que la teoría G3 $1.04$ vs $1.39 kcal\cdot mol^{-1}$. Se dijo que G4 (MP2) funcionaba ``razonablemente bien'' para la termoquímica de los metales de transición, especies que presentan problemas para la química computacional [184]. G4(MP2) y G3(MP2) pueden manejar moléculas con hasta 16 átomos pesados.

Estos siete pasos se utilizan para ensamblar una energía molecular como la suma de varias diferencias de energía y un incremento de energía empírico final (la "corrección de nivel superior") en función del número de electrones apareados y no apareados. La energía G4(MP2) es esencialmente un tipo de energía CCSD(T)/6–31G* realizada en una geometría B3LYP/ 6–31G(2df,p), con una escala B3LYP/6–31G(2df,p) ZPE y una corrección de energía empírica, pero un cálculo tan directo sería más lento que dividirlo en los pasos utilizados aquí. Una forma en que G4(MP2) ahorra tiempo es reemplazando un cálculo de MP3 y MP4 por uno de MP2. Una mejora clave en G4/G4(MP2) sobre G3/G3(MP2) es el reemplazo del método de correlación cuadrática CI por el método de conglomerados acoplados (Sección 5.4.3); este cambio en particular no alteró la precisión del conjunto de pruebas de moléculas, pero presumiblemente mejora la confiabilidad, como “. . .el método QCISD(T) tiene fallas bastante dramáticas, lo que no ocurre con el método CCSD(T)” [181]. Véase también Hrusak et al. para una comparación de CI cuadrático y clúster acoplado [103]. En el método G3(MP2), el principal cambio con respecto a G3 es que los cálculos de MP2 reemplazan a los de MP4 [183]. Debido a las correcciones de energía empírica en los métodos multipaso gaussianos, no son completamente ab initio, sino algo semiempíricos, excepto cuando estas correcciones se anulan. Esta cancelación ocurre, por ejemplo, al calcular las afinidades de los protones como la diferencia de energía de las especies protonadas y no protonadas, donde las correcciones de la órbita de espín y el número de electrones de espín α y β son los mismos en ambos lados de la ecuación. Tomaremos G4(MP2) como el método gaussiano de elección, con un buen compromiso entre precisión y velocidad, pero también nos referiremos a los cálculos G3(MP2), debido a su velocidad y precisión competitivas y su amplio legado. Estos métodos gaussianos, y otros no mencionados aquí, así como los métodos CBS (a continuación) son revisados ​​(2012) por Peterson, Feller y Dixon, con énfasis en termoquímica, estructuras y frecuencias; estos autores reconocen ca. 4 kJ mol"1 (1 kJ mol"1) como precisión química aceptada en termoquímica [185]. Aquí hay una comparación de los métodos gaussianos y un método CBS, usando 1,4-benzoquinona (p-benzoquinona, O¼C6H4¼O) y mostrando tiempos (realizados en una computadora antigua de 2013) y, como indicación de precisión, entalpías de formación calculadas por el método de atomización; tenga en cuenta que el cálculo de G4 tomó nueve veces más tiempo (160/18) que el G4 (MP2), y ambos dieron casi la misma entalpía de formación: la entalpía de formación aceptada en la literatura es "122.6(3.8 kJ mol"1 [186]. Los primeros cuatro valores calculados acomodan el valor experimental dentro de 1 kJ mol"1, sin embargo, el valor CBS-QB3 es 3 kJ mol"1 por encima del mayor error estimado Pero uno no debe generalizar a partir de una muestra de un compuesto.
