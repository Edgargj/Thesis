...
\begin{tabular}{||l|c|c||}
\hline 
Método & $\Delta_{f}H/(kJmol^{-1})$ & minutos/tiempo relativo \\ 
\hline 
G4 & -117.5 & 160/21 \\ 
\hline 
G4(MP2) & -118.5 & 18/2.4 \\ 
...
La entalpía de atomización, $\Delta_{a} H^{\circleddash}_{T}$ a T = 0 K \textit{ab initio}, del metanol es la diferencia de entalpía entre los átomos de la molécula y el metanol. Para calcular $\Delta_{f}H^{\circleddash}_{0}$ necesitamos la entalpía de formación a T = 0 K de los átomos de C, H y O y la entalpía de atomización a T = 0 K del metanol (ecuaciones \ref{eq:3.13} y \ref{eq:3.14}). Las entalpías de atomización del hidrógeno y el oxígeno se pueden calcular por un método \textit{ab initio}, pero no de forma fiable y precisa. Para mantener la coherencia, utilizaremos valores experimentales de las tres entalpías de atomización elementales, como recomiendan Nicolaides et al\cite{Nicolaides1996}. De la ecuación \ref{eq:3.15}, la entalpía de atomización T = 0 K del metanol es simplemente las suma de las entalpías \textit{ab initio} de los átomos que la constituyen menos la entalpía \textit{ab initio} corregida por ZPE del metanol:

\begin{equation}
\Delta_{f} H^{\circleddash}_{0}(CH_3OH) + \Delta_{a} H^{\circleddash}_{0} (CH_{3}OH) = \Delta_{f} H^{\circleddash}_{0} (C(^{3}P) + 4H(^{2}S) + O(^{3}S))
\label{eq:3.13}
\end{equation}\\

\begin{equation}
\Delta_{f} H^{\circleddash}_{0}(CH_3OH) = \Delta_{f} H^{\circleddash}_{0} (C(^{3}P) + 4H(^{2}S) + O(^{3}S)) - \Delta_{f} H^{\circleddash}_{0} (CH_3OH)
\label{eq:3.14}
\end{equation}\\

\begin{equation}
\Delta_{a} H^{\circleddash}_{0}(CH_3OH) = \Delta_{0K} E^{total} (C(^{3}P) + 4H(^{2}S) + O(^{3}S)) - \Delta_{0K} E^{total} (CH_3OH)
\label{eq:3.15}
\end{equation}\\
...
\caption[Diagrama de entalpía de atomización de metanol.]{El metanol se atomiza (conceptualmente) a T = 0 K en los átomos que lo constituyen, es decir,  carbono, hidrógeno y oxígeno (en sus estados electrónicos fundamentales). Además, los átomos de la molécula en sus estados estándar también son usados para formar dicha molécula. La entalpía de formación del metanol a T = 0 K, $\Delta_{f} H^{\circleddash}_{0}(CH_3OH)$, se obtiene al igualar la energía necesaria para generar los átomos a través del metanol ($ \Delta_{f} H^{\circleddash}_{0}(CH_3OH)$ + $ \Delta_{a} H^{\circleddash}_{0}(CH_3OH)$) a la energía para hacerlo directamente desde sus elementos en sus estados estándar \cite{Lewars2016}.}
\label{atm-metanol}
\end{center}
\end{figure}
...
\hline 
\end{tabular}
\caption{Entalpías de formación experimentales reportadas por Nicolaides et al \cite{Nicolaides1996}.}
\label{Nicolaides-table}
\end{center}
\end{table}

Posteriormente, para calcular $\Delta_{a} H^{\circleddash}_{0}(CH_3OH)$ haremos uso de la ecuación \ref{eq:3.15} que toma el valor de $\Delta E^{total}_{0K}$ para los átomos de  C, H, y O. En lugar del método G2 que fue empleado por Nicolaides et al \cite{Nicolaides1996}, usaremos el método G4 de \textit{Gaussian09}. Los valores obtenidos para cada átomo y su respectiva molécula a T = 0 K se muestran en la tabla \ref{G4-table}.\\
...
El valor de metanol de $-115.651767 h$ podría llamarse el ``cero absoluto'' de la entalpía del metanol, es decir, la entalpía relativa a la de los núcleos disociados y electrones. Esta entalpía absoluta se utilizará en el cálculo de la entalpía de formación. De la ecuación \ref{eq:3.15} es posible calcular la energía de atomización de metanol a T = 0 K:

\begin{multline}
\Delta_{a} H^{\circleddash}_{0}(CH_3OH) = -37.834168 + 4(-0.501420)-75.045501-(-115.651767) h =...\\
...
Las entalpías experimentales de los respectivos elementos también es reportada por Nicolaides et al.\cite{Nicolaides1996} en $kJ mol^{-1}$, veáse la tabla \ref{Nicolaides-exp-table}.
...
\hline 
\end{tabular}
\caption{Entalpías experimentales de los elementos reportadas por Nicolaides et al\cite{Nicolaides1996}.}
...
frecuencias vibracionales \textit{ab initio} a menudo tienen un error de $+5\%$ a $+10\%$. Para compensar este error, las frecuencias calculadas generalmente se multiplican por factores de escala empíricos. Las recomendaciones más recientes son las de Scott et al \cite{Scott1996}. Por ejemplo, sugieren escalar las frecuencias HF/6-31G* en 0.8953 para predecir espectros de vibración (es decir, frecuencias fundamentales), en 0.9135 para el cálculo de ZPE, en 0.8905 para predecir diferencias de entalpía [H°(298,15) - H°(0)].\\
