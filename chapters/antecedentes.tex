...
Por definición, la \textit{entalpía estándar de formación} de cualquier elemento es cero, porque no se necesita una reacción de formación cuando el elemento ya se encuentra en su estado estándar. Así, los valores de $\Delta_{f}H^{\circ}$ para el $C$(grafito), $H_{2}$(g), $O_{2}$(g) y los estados estándar de otros elementos son cero \cite{Brown2014, Chang2008}.

\begin{equation}
\Delta_{f}H^{\circ} (grafito) = 0
\label{eq:3.6}
\end{equation}

\begin{equation}
\Delta_{f}H^{\circ} (H_{2}) = 0
\label{eq:3.7}
\end{equation}

\begin{equation}
\Delta_{f}H^{\circ} (O_{2}) = 0
\label{eq:3.8}
\end{equation}

Como se mecionó antes, no podemos determinar el valor absoluto de la entalpía de una sustancia. Sólo se pueden calcular valores relativos a una referencía arbitraria. En termodinámica, lo que nos interesa, son los cambio de $H$, aunque todo valor de $\Delta_{f} H^{\circ}$ asignado arbitrariamente a un elemento funcionaría, con ceros los cálculos se simplifican \cite{Chang2008}. Los valores de $\Delta_{f}H^{\circ}$ pueden obtenerse con un método directo o indirecto, a continuación se describen.
...
\textbf{Método indirecto}. La mayor parte de los compuestos no se pueden sintetizar directamente a partir de sus elementos. En algunos casos, las reacciones suceden con demasida lentitud o no llegan a concretarse. Por lo tanto, el valor de $\Delta_{f}H^{\circ}$ se puede determinar basado en la ley de Hess \cite{Chang2008}. 
...
\begin{equation}
\begin{aligned}
& CO(g) + \frac{1}{2} O_{2}(g) \longrightarrow CO_{2}(g) & \Delta_{f}H^{\circ} = -283.0 kJ\cdot mol^{-1}
\end{aligned}
\label{eq:3.10}
\end{equation}
...
\begin{equation}
\begin{aligned}
& CO_{2}(g) \longrightarrow CO(g) + \frac{1}{2} O_{2}(g) & \Delta_{f}H^{\circ} = 283.0 kJ\cdot mol^{-1}
\end{aligned}
\label{eq:3.11}
\end{equation}
...
\begin{equation}
\begin{aligned}
& C(grafito) + \frac{1}{2} O_{2}(g) \longrightarrow CO_{2}(g) & \Delta_{f}H^{\circ} = -110.5 kJ\cdot mol^{-1}
\end{aligned}
\label{eq:3.12}
\end{equation}
...
De los 3 enfoques anteriores, el método de atomización da mejores resultados, especialmente para moléculas orgánicas y es el más sencillo, porque se basa en la ruptura de enlaces de una molécula para obtener a sus átomos en fase gasesosa\cite{Nicolaides1996}. Veamos el principio del método de atomización con la molécula de metanol. \\

La entalpía de atomización $\Delta_{a0} H^{\circleddash}$ a T = 0 K \textit{ab initio} del metanol, es la diferencia de entalpía entre los átomos de la molécula y el metanol. Para calcular $\Delta_{f0}H^{\circleddash}$ necesitamos la entalpía de formación a T = 0 K de los átomos de C, H y O y la entalpía de atomización a T = 0 K del metanol (ecuaciones \ref{eq:3.13} y \ref{eq:3.14}). Las entalpías de atomización del hidrógeno y el oxígeno se pueden calcular por un método \textit{ab initio}, pero no de forma fiable y precisa. Para mantener la coherencia, utilizaremos valores experimentales de las tres entalpías de atomización elementales, como recomiendan Nicolaides y otros autores \cite{Nicolaides1996}. De la ecuación \ref{eq:3.15}, la entalpía de atomización T = 0 K del metanol es simplemente las suma de las entalpías \textit{ab initio} de los átomos que la constituyen menos la entalpía \textit{ab initio} corregida por ZPE del metanol:

\begin{equation}
\Delta_{f0} H^{\circleddash}(CH_3OH) + \Delta_{a0} H^{\circleddash} (CH_{3}OH) = \Delta_{f0} H^{\circleddash} (C(^{3}P) + 4H(^{2}S) + O(^{3}S))
\label{eq:3.13}
\end{equation}\\

\begin{equation}
\Delta_{f0} H^{\circleddash}(CH_3OH) = \Delta_{f0} H^{\circleddash} (C(^{3}P) + 4H(^{2}S) + O(^{3}S)) - \Delta_{f0} H^{\circleddash} (CH_3OH)
\label{eq:3.14}
\end{equation}\\

\begin{equation}
\Delta_{a0} H^{\circleddash}(CH_3OH) = \Delta_{0K} E^{total} (C(^{3}P) + 4H(^{2}S) + O(^{3}S)) - \Delta_{0K} E^{total} (CH_3OH)
\label{eq:3.15}
\end{equation}\\
...
\caption{Diagrama de entalpía de atomización de metanol. El metanol se atomiza (conceptualmente) a T = 0 K en los átomos que lo constituyen, es decir,  carbono, hidrógeno y oxígeno (en sus estados electrónicos fundamentales). Además, los átomos de la molécula en sus estados estándar también son usados para formar dicha molécula. La entalpía de formación del metanol a T = 0 K, $\Delta_{f0} H^{\circleddash}(CH_3OH)$, se obtiene al igualar la energía necesaria para generar los átomos a través del metanol ($ \Delta_{f0} H^{\circleddash}(CH_3OH)$ + $ \Delta_{a0} H^{\circleddash}(CH_3OH)$) a la energía para hacerlo directamente desde sus elementos en sus estados estándar \cite{Lewars2016}.}
\label{atm-metanol}
\end{center}
\end{figure}
...
\begin{multline}
\Delta_{a0} H^{\circleddash}(CH_3OH) = -37.834168 + 4(-0.501420)-75.045501-(-115.651767) h =...\\
...
\begin{multline}
\Delta_{f0} H^{\circleddash}(CH_3OH) = 711.2 + 4(216.035) + 246.8 -2012.230459 kJ mol^{-1} = -189.1 kJ mol^{-1}
\label{eq:3.17}
\end{multline}\\
...
\begin{multline}
\Delta_{f} H^{\circleddash}(CH_3OH) = \Delta_{f0} H^{\circleddash}(CH_{3}OH) + \Delta \Delta H^{\circleddash} (CH_{3}OH) \\ - (\Delta \Delta H^{\circleddash} (C) + 2 \Delta \Delta H^{\circleddash}(H_{2}) + \frac{1}{2} \Delta \Delta H^{\circleddash}(O_{2}))
\label{eq:5.20}
\end{multline}\\

\begin{multline}
\Delta_{f} H^{\circleddash}(CH_3OH) = -190.1126 + 11.23188  \\ - (1.05100 + 2(8.46700) + \frac{1}{2} (8.67000))\:kJ\cdot mol^{-1}
\label{eq:5.21}
\end{multline}\\

\begin{equation}
\Delta_{f} H^{\circleddash}(CH_3OH) = -201.21\:kJ\cdot mol^{-1}
\label{eq:5.22}
\end{equation}\\

El valor experimental a T = 298.15 K aceptado es de 205 $\pm$ 10 $kJmol^{-1}$\cite{Afeefy2009}. Por lo tanto, La cifra calculada está dentro del error experimental esperado \cite{Lewars2016}.
...
frecuencias vibracionales \textit{ab initio} a menudo tienen un error de $+5\%$ a $+10\%$. Para compensar este error, las frecuencias calculadas generalmente se multiplican por factores de escala empíricos. Las recomendaciones más recientes son las de Scott y otros autores \cite{Scott1996}. Por ejemplo, sugieren escalar las frecuencias HF/6-31G* en 0.8953 para predecir espectros de vibración (es decir, frecuencias fundamentales), en 0.9135 para el cálculo de ZPE, en 0.8905 para predecir diferencias de entalpía [H°(298,15) - H°(0)].\\
...
Actualmente, las computadoras pueden realizar cálculos y tomar decisiones lógicas con una rapidez inimaginable 
...
A los programas que se ejecutan en una computadora se les denomina software. A las partes de una computadora que consisten en varios dispositivos se les conoce como hardware (teclado, pantalla, ratón, discos duros, memoria, unidades de procesamiento) \cite{Deitel2014}. 
...
el cual se define según su diseño de hardware. Por lo general, los lenguajes máquina consisten en cadenas de números (que finalmente se reducen a 1s y 0s). 
...
\item \textbf{Abstracción}. Las características esenciales y la funcionalidad de una clase se colocan en un solo lugar y los detalles de cómo funcionan no son importantes para el usuario de la clase. Por ejemplo, si está utilizando una biblioteca de sistema lineal para resolver ecuaciones matriciales, no debería necesitar conocer los detalles precisos de cómo se disponen las matrices en la memoria o el orden exacto en que un solucionador numérico realiza sus operaciones. Solo debe saber cómo usar la funcionalidad de la biblioteca.
\item \textbf{Encapsulación}. La implementación de un objeto se mantiene oculta para el usuario de la clase. No se trata solo de claridad (abstrayendo los detalles). También se trata de evitar que el usuario modifique accidentalmente el funcionamiento interno de, por ejemplo, un solucionador lineal, evitando que funcione de manera efectiva.
...
La mayoría de los programas de C++ para computación científica se pueden escribir de manera muy efectiva utilizando solo una fracción de las capacidades totales del lenguaje. En este trabajo nos centraremos en los aspectos de C++ que es más probable que se utilice o encuentre en el código de otro programador, 
para aplicaciones informáticas científicas \cite{Pitt2017}. 
